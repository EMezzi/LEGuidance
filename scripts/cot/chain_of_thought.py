import os
import json
from prompts.cot.chain_of_thought import *
...
from schemas.pydantic_schemas import CoTAnswer
import base64
        ...
        "inputSchema": {
            "json": {
                ...
                "properties": {
                    ...
                    "reasoning": {
                        "type": "string",
                        "description": "The reasoning to arrive at the final answer"
                    },
                    "entity": {
                        "type": "string",
                ...
                "required": ["contains", "reasoning", "entity", "confidence"],
            }
        },
    }
}
...
class CoTAgent:
    ...
    def cot_final_answer(self, model, question_text, paragraphs_text, images_text, images_inputs, tables_text):
        ...
        if model == "global.amazon.nova-2-lite-v1:0":
            ...
            formatted_user_text = user_prompt_cot.format(
            ...
            system_prompts = [{"text": system_prompt_cot}]
            ...
            try:
                response = self.bedrock_client.converse(
                    ...
                    toolConfig={
                        "tools": [answer_tool],
                        "toolChoice": {
                            "tool": {"name": "get_answer_cot"}  # Forces the tool call
                        }
                    }
                )
                    ...
                    input=[
                        ...
                        },
                        {
                            ...
                            "content": [
                                {
                                    "type": "input_text",
                                    "text": user_prompt_cot.format(question_text=question_text, images_text=images_text,
                                                                   paragraphs_text=paragraphs_text,
                                                                   tables_text=tables_text)
            ...
            for i in range(0, len(images_inputs), 3):
                ...
                try:
                    ...
                    response = self.bedrock_client.converse(
                        modelId=model,
                        messages=messages,
                        system=[{"text": system_prompt_cot}],
                        inferenceConfig={"temperature": 0.0, "maxTokens": 1000},
                        toolConfig={
                            "tools": [answer_tool],
                            "toolChoice": {
                                "tool": {"name": "get_answer_cot"}  # Forces the model to use the tool
                            }
                        }
                    )
            ...
            try:
                ...
                - 'reasoning': string (the reasoning to arrive at the final answer)
