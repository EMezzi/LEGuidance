import os
import json
from prompts.pp.planning_prompting import *
...
from schemas.pydantic_schemas import PPAnswer
import base64
        ...
        "inputSchema": {
            "json": {
                ...
                "properties": {
                    "contains": {
                        "type": "boolean",
                        "description": "True if data contains the answer"
                    },
                    "plan": {
                        "type": "string",
                        "description": "The high-level strategy (Phase 1)"
                    },
                    "execution": {
                        "type": "string",
                        "description": "Step-by-step extraction and logic (Phase 2)"
                    },
                    "entity": {
                        "type": "string",
                        "description": "The final concise answer or NONE"
                    },
                    "confidence": {
                        "type": "number",
                ...
                "required": ["contains", "plan", "execution", "entity", "confidence"],
            }
        },
    }
}
...
class PPAgent:
    ...
    def pp_final_answer(self, model, question_text, paragraphs_text, images_text, images_inputs, tables_text):
        ...
        if model == "global.amazon.nova-2-lite-v1:0":
            ...
            formatted_user_text = user_prompt_pp.format(
            ...
            system_prompts = [{"text": system_prompt_pp}]
            ...
            try:
                response = self.bedrock_client.converse(
                    ...
                    toolConfig={
                        "tools": [answer_tool],
                        "toolChoice": {
                            "tool": {"name": "get_answer_pp"}  # Forces the tool call
                        }
                    }
                )
                    ...
                    input=[
                        ...
                        },
                        {
                            ...
                            "content": [
                                {
                                    "type": "input_text",
                                    "text": user_prompt_pp.format(question_text=question_text, images_text=images_text,
                                                                   paragraphs_text=paragraphs_text,
                                                                   tables_text=tables_text)
            ...
            for i in range(0, len(images_inputs), 3):
                ...
                prompt_text = user_prompt_pp.format(
                    question_text=question_text,
                ...
                try:
                    ...
                    response = self.bedrock_client.converse(
                        modelId=model,
                        messages=messages,
                        system=[{"text": system_prompt_pp}],
                        inferenceConfig={"temperature": 0.0, "maxTokens": 1000},
                        toolConfig={
                            "tools": [answer_tool],
                            "toolChoice": {
                                "tool": {"name": "get_answer_pp"}  # Forces the model to use the tool
                            }
                        }
                    )
            ...
            try:
                ...
                prompt_text += """\n### Answer by returning: 
                - 'contains': boolean (true if data contains the answer)
                - 'plan': string (the high-level strategy (Phase 1))
                - 'execution': string (step-by-step extraction and logic (Phase 2)
                - 'entity': string (the final concise answer or NONE if answer is not found)
